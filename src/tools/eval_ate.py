...
def evaluate_ate_report(first_list, second_list, plot="", _args="",rel=False):
    # parse command line
    parser = argparse.ArgumentParser(
        description='This script computes the absolute trajectory error from the ground truth trajectory and the estimated trajectory.')
    # parser.add_argument('first_file', help='ground truth trajectory (format: timestamp tx ty tz qx qy qz qw)')
    ...
    if args.verbose:
        if rel:
            print("this is relative pose evaluation!")
        print("compared_pose_pairs %d pairs" % (len(trans_error)))
    ...
    # }
    return
def evaluate(poses_gt, poses_est, plot,report=False,rel=False):
    ...
    if report==True:
        evaluate_ate_report(poses_gt, poses_est, plot,rel=rel)
    else:
...
def pose_evaluation(poses_gt, poses_est, scale, path_to_save, i, img='pose', rel=False):
    N = len(poses_est)-1
    ...
    results = evaluate(poses_gt, poses_est, plot=plt_path,report=True,rel=rel)
    # results['Name'] = i
