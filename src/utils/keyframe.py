...
class KeyFrameDatabase(object):
    def __init__(self, config, H, W, num_kf, num_planes, num_rays_to_save, device) -> None:
        self.config = config
        self.keyframes = {}
        self.device = device
        self.rays = torch.zeros((num_kf, num_rays_to_save, 7))
        self.planes = torch.zeros( num_planes, 4)
    ...
    def attach_ids(self, frame_ids):
        ...
        if self.frame_ids is None:
            ...
            tem = torch.cat([self.frame_ids, frame_ids], dim=0)
            self.frame_ids = tem

    def add_keyframe(self, batch, filter_depth=False,num_plane_everyframe=3):
        '''
        Add keyframe rays to the keyframe database
        '''
        # batch direction (Bs=1, H*W, 3)
        idx,color,depth,direc,planes=batch

        rays = torch.cat([direc.squeeze(), color.squeeze(), depth.squeeze()[..., None]], dim=-1).float()
        ...
        self.planes[(len(self.frame_ids) - 1)*num_plane_everyframe:(len(self.frame_ids))*num_plane_everyframe,:]=planes

    def sample_global_rays(self, bs,bs_plane=20):
        '''
        Sample rays from self.rays as well as frame_ids
        '''
        num_kf = self.__len__()
        #sample rays and idx of rays
        ...
        #sample planes and idx of planes
        if num_kf>=1+bs_plane/3:
            idxs_plane = torch.tensor(random.sample(range(3,num_kf * 3), bs_plane))
            sample_planes = self.planes[:num_kf*3].reshape(-1, 4)[idxs_plane]
            frame_ids_planes = self.frame_ids[idxs_plane // 3]
            return sample_rays, frame_ids,sample_planes,frame_ids_planes
        else:
