...
class Renderer(object):
    ...
    """
    def __init__(self, cfg, ismap, ray_batch_size=10000):
        ...
        self.bound = ismap.bound.to(ismap.device, non_blocking=True)
    ...
    def render_batch_ray(self, all_planes, decoders, rays_d, rays_o, device, truncation, gt_depth=None,onlydepth=False):
        """
        ...
        gt_nonezero = gt_depth[gt_mask]#[n_rays,1]

        ## Sampling points around the gt depth (surface)
        gt_depth_surface = gt_nonezero.expand(-1, n_importance)#[n_rays,8]
        ...
        if not gt_mask.all():
            with torch.no_grad():
                ...
                if decoders.use_sdffeatur:
                    sdf_uni, _ = decoders.get_raw_sdf(pts_uni_nor, all_planes)
                else:
        ...
        if onlydepth:
            ...
            return rendered_depth, rendered_rgb, sdfs, z_vals#
